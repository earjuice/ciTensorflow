...
namespace tf {
	...
	*/
	Status loadGraph(const string &graph_file_name,
		...
		tensorflow::SessionOptions opts = tensorflow::SessionOptions();
		
		//opts.config.add_device_filters("Nvidia");
	...
	*/
		...
		std::vector<double>& rScore)
	{
		for (int j = 0; j < idxs.size(); j++)
		{
			rYmin.push_back(boxes(0, idxs.at(j), 0));
			rXmin.push_back(boxes(0, idxs.at(j), 1));
			rYmax.push_back(boxes(0, idxs.at(j), 2));
			rXmax.push_back(boxes(0, idxs.at(j), 3));
			rScore.push_back(scores(idxs.at(j)));
		}
	}
